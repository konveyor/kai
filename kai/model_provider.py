...
from langchain_core.messages import BaseMessageChunk, HumanMessage, SystemMessage
...
class IBMGraniteModel(ModelProvider):
    def __init__(self, model_id: str = "ibm/granite-20b-code-instruct-v1") -> None:
...
class IBMLlamaModel(ModelProvider):
    def __init__(self, model_id: str = "codellama/codellama-34b-instruct") -> None:
        if os.environ.get("GENAI_KEY") is None:
            raise Exception(
                "Must set GENAI_KEY in environment if using IBMGraniteModel"
            )
    ...
    def get_prompt_builder(self) -> prompt_builder.Config:
        return self.prompt_builder_config
