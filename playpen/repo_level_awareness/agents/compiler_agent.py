from playpen.repo_level_awareness.api import Task, TaskResult, Agent
...
class MavenCompilerAgent(Agent):
    ...
    def execute_task(self, task: Task) -> TaskResult:
        ...
        with open(task.file) as f: src_file_contents= f.read()
        ...
        aimessage = self.__llm.invoke([self.system_message, HumanMessage(content=content)])
        ...
        with open(task.file, "w") as f: f.write(resp.java_file)
    ...
    def parse_llm_response(self, message: BaseMessage) -> MavenCompilerLLMResponse:
        ...
        return MavenCompilerLLMResponse(reasoning=reasoning, java_file=java_file, addional_information=additional_details)
